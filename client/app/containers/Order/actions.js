...
export const fetchAccountOrders = (page = 1) => {
  return async (dispatch, getState) => {
    try {
      ...
      const response = await axios.get(`${process.env.REACT_APP_API_URL}/order/me`, {
        params: {
          page: page ?? 1,
          limit: 20
        }
...
export const searchOrders = filter => {
  return async (dispatch, getState) => {
    try {
      ...
      const response = await axios.get(`${process.env.REACT_APP_API_URL}/order/search`, {
        params: {
          search: filter.value
        }
      });
...
export const fetchOrder = (id, withLoading = true) => {
  return async (dispatch, getState) => {
    try {
      ...
      const response = await axios.get(`${process.env.REACT_APP_API_URL}/order/${id}`);
...
export const cancelOrder = () => {
  return async (dispatch, getState) => {
    try {
      ...
      await axios.delete(`${process.env.REACT_APP_API_URL}/order/cancel/${order._id}`);
  ...
  return async (dispatch, getState) => {
    try {
      ...
      const response = await axios.put(
        `${process.env.REACT_APP_API_URL}/order/status/item/${itemId}`,
        {
          orderId: order._id,
          cartId: order.cartId,
          status
...
export const addOrder = () => {
  return async (dispatch, getState) => {
    try {
      ...
      if (cartId) {
        const response = await axios.post(`${process.env.REACT_APP_API_URL}/order/add`, {
          cartId,
          total
        });

...
export const resetPassword = token => {
  return async (dispatch, getState) => {
    try {
      ...
      const response = await axios.post(`${process.env.REACT_APP_API_URL}/auth/reset/${token}`, user);
...
export const resetAccountPassword = () => {
  return async (dispatch, getState) => {
    try {
      ...
      const response = await axios.post(`${process.env.REACT_APP_API_URL}/auth/reset`, user);
      const successfulOptions = {
        title: `${response.data.message}`,
        position: 'tr',
        autoDismiss: 1

...
export const approveReview = review => {
  return async (dispatch, getState) => {
    try {
      await axios.put(`${process.env.REACT_APP_API_URL}/review/approve/${review._id}`);
...
export const rejectReview = review => {
  return async (dispatch, getState) => {
    try {
      await axios.put(`${process.env.REACT_APP_API_URL}/review/reject/${review._id}`);
...
export const fetchProductReviews = slug => {
  return async (dispatch, getState) => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_API_URL}/review/${slug}`);
